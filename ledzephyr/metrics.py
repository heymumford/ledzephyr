...
class MetricsCalculator:
    ...
    def calculate_metrics(
        ...
        project = self.client.get_jira_project(project_key)
        ...
        zephyr_executions = self.client.get_test_executions(
            project_key, zephyr_test_ids, "zephyr"
        )
        qtest_executions = self.client.get_test_executions(
            project_key, qtest_test_ids, "qtest"
        )
        ...
        trend_data = self._calculate_trend_data(
            project_key, teams_source, end_date
        )
    ...
    def _calculate_trend_data(
        ...
        for week in range(1, 5):
            ...
            try:
                ...
                zephyr_tests = self.client.get_zephyr_tests(
                    project_key, week_start, week_end
                )
                qtest_tests = self.client.get_qtest_tests(
                    project_key, week_start, week_end
                )
        ...
        if trend_data.week_1 and trend_data.week_4:
            trend_data.adoption_trend = (
                trend_data.week_1.get("adoption_ratio", 0) - 
                trend_data.week_4.get("adoption_ratio", 0)
            )
            trend_data.coverage_trend = (
                trend_data.week_1.get("coverage_parity", 0) - 
                trend_data.week_4.get("coverage_parity", 0)
            )
            trend_data.activity_trend = (
                trend_data.week_1.get("active_users", 0) - 
