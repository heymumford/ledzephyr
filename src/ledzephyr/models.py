...
class ProjectMetrics(BaseModel):
    ...
    adoption_ratio: float = Field(
        0.0, description="qTest adoption ratio (qtest_tests / total_tests)"
    )
    ...
    team_metrics: dict[str, "TeamMetrics"] = Field(
        default_factory=dict, description="Metrics broken down by team"
    )
...
class TestCaseModel(BaseModel):
    ...
    # Team identification
    component: str | None = Field(None, description="Component")
    ...
    # Execution data
    last_execution: datetime | None = Field(None, description="Last execution date")
    execution_status: str | None = Field(None, description="Last execution status")
