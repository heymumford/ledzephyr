...
from ledzephyr.models import APIConnectionStatus, JiraProject, TestCase
...
class APIClient:
    ...
    def __init__(self, config: Config):
        ...
        self._http_client = httpx.Client(
            ...
            headers={"User-Agent": "ledzephyr/0.1.0"}
        )
    ...
    def test_qtest_connection(self) -> bool:
        ...
        try:
            ...
            response = self._make_request(
                "GET",
                f"{self.config.qtest_url}/api/v3/users/me",
                headers=headers
            )
            return response.status_code == 200
        except Exception as e:
    ...
    def get_jira_project(self, project_key: str) -> JiraProject:
        ...
        response = self._make_request(
            "GET",
            f"{self.config.jira_url}/rest/api/2/project/{project_key}",
            auth=auth
        )
    ...
    def get_qtest_tests(
        ...
        try:
            ...
            projects_response = self._make_request(
                "GET",
                f"{self.config.qtest_url}/api/v3/projects",
                headers=headers
            )
            ...
            for item in data.get("items", []):
                test = TestCase(
                    ...
                    updated=datetime.fromisoformat(item["last_modified_date"].replace("Z", "+00:00")),
